...
Player::Player(GLWindow& glWindow, glm::vec3 position, glm::mat4 projection) {
	...
	// Ray Casting
	//rayCast = RayCasting(glWindow, camera, projection); //NOT WORKING ????
}
...
void Player::update(GLfloat deltaTime) {
	...
	//std::cout << rayCast.getCurrentRay().x << '\n';
	//std::cout << rayCast.getCurrentRay().y << '\n';
	//std::cout << rayCast.getCurrentRay().z << '\n';
}
